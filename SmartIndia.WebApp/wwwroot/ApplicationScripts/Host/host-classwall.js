            ...
            success: function (data) {
                ...
                $.each(data, function (i, item) {
                    if (data[i].isPublished==1) {
                        var color = '<div class="color-legend col-leg-yellow"></div>';
                    } else {
                        color = '<div class="color-legend col-leg-seablue"></div>';
                    }

                    trHTML += '<li class="list-group-item justify-content-between ocr-list-group"> '
                        + '<div>' + color +'</div><div class="sm-card-title" data-toggle="tooltip" data-placement="bottom" title="' + data[i].title + '">'

                        + ' <a href="' + ClientURL +'/Hosts/ClassWall/ClassWallDetail?SID=' + data[i].schedularId + '" >' + data[i].title + ' ' + "(" + '' + data[i].topics + '' + ")" + ' </a>'
